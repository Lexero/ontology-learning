...
def parse_itemset(itemset):
    ...
    if itemset[0].split('_')[0] == 'label':
        cluster = int(itemset[0].split('_')[1])
        feature, value = itemset[1].split('_')
    else:
        cluster = int(itemset[1].split('_')[1])
        feature, value = itemset[0].split('_')
...
class GridSearch:
    ...
    def fit(self, X):
        ...
        if self.scoring == silhouette_score:
            ...
            print(find_nearest_idx(array(self.result_grid)[:, 1], 1))
...
class OntoClusterer:
    ...
    def fit(self, X, data : pd.DataFrame, grid_params = None, scoring = None, **kwargs) -> owl.Ontology:
        ...
        if self.algorithm == 'xmeans':
            ...
            if grid_params == None:
                self.model = AffinityPropagation(random_state = None, **kwargs)
            else:
        ...
        with onto:
            ...
            for feature_name in data.select_dtypes(['object']).columns:
                new_feature = types.new_class(f'feature_{feature_name}', (owl.Thing, ), )
                ...
                for pair in frequent_itemsets['itemsets']:
                    ...
                    exec(f"new_value = types.new_class(value, (onto.feature_{feature} >> str, ))")
                ...
                if len(apriori_data) > 0:
                    ...
                    for pair in ar[['antecedents', 'consequents']].values:
                        ...
                        exec(f"new_value = types.new_class(value, (onto.feature_{feature} >> str, ))")
                        exec(f"onto.Cluster{cluster}.is_a.append(onto.has_{feature}.only(new_value))")
